...
A admissão segue a ordem da lista de processos, ordenada por tempo de chegada e,
em empate, por `id`. Essa fila é implícita e não bloqueia a admissão inteira: se
um processo mais antigo ainda não couber nos recursos disponíveis, o dispatcher
continua avaliando os próximos processos e pode admitir um processo posterior
que caiba naquele momento. Essa escolha melhora o aproveitamento de memória e
discos sem alterar as filas de escalonamento de CPU.
...
Glossário rápido do relatório:

- `admissao`: processo saiu de `NEW` porque conseguiu reservar memória e discos;
- `tempo-real`: fila de processos de prioridade `0`, escalonados em FCFS;
- `usuario0`, `usuario1`, `usuario2`: filas de feedback dos processos de
  usuário, da maior para a menor prioridade;
- `io`: processos bloqueados aguardando início de I/O em algum disco;
- `discos reservados`: discos comprometidos por processos admitidos, mesmo que
